...
function CheckoutConfirmation({ searchParams }: { searchParams?: Record<string, string | undefined> }) {
  ...
  const confirmOrder = api.checkout.confirmOrder.useMutation({
    onSuccess: () => {
      handleRedirect();
      setMessage('You should be redirected momentarily.');
      setSuccess(true);
    },
    onError: (error) => {
      setMessage(`Error confirming order: ${error.message}. Please try again.`);
    },
  });
  ...
  useEffect(() => {
    if (searchParams?.sessionId && searchParams?.orderId) {
      ...
      setMessage('Missing Order or Session ID. Please try again.');
    }

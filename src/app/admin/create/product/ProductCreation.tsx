...
function Color({ color }: { color: string }) {
  return (
    <span
      className={`h-4 w-4 rounded-sm border border-slate-800 shadow-md transition duration-200 ease-in-out dark:border-slate-200 ${color === 'black' ? 'bg-black' : color === 'white' ? 'bg-white' : `bg-${color}-500`}`}></span>
  );
}
...
function GenderSelection({ setFormState }: { setFormState: React.Dispatch<React.SetStateAction<FormState>> }) {
  return (
    <>
      ...
      <select
        name="gender"
        className="h-20 w-full overflow-hidden rounded-sm bg-slate-300 px-4 py-2 dark:bg-slate-700"
        multiple
        onChange={(e) => {
